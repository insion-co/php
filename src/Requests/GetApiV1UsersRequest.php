<?php

namespace Insion\Requests;

use Insion\Core\Json\JsonSerializableType;
use Insion\Types\GetApiV1UsersRequestStatus;

class GetApiV1UsersRequest extends JsonSerializableType
{
    /**
     * @var ?int $limit Maximum number of items to return.
     */
    public ?int $limit;

    /**
     * @var ?string $startingAfter Return items after this Insion ID. Cannot be used with ending_before.
     */
    public ?string $startingAfter;

    /**
     * @var ?string $endingBefore Return items before this Insion ID. Cannot be used with starting_after.
     */
    public ?string $endingBefore;

    /**
     * @var ?string $clientId Filter by your user identifier.
     */
    public ?string $clientId;

    /**
     * @var ?string $email Filter by user email.
     */
    public ?string $email;

    /**
     * @var ?value-of<GetApiV1UsersRequestStatus> $status Filter by user action status.
     */
    public ?string $status;

    /**
     * @var ?string $user Filter by Insion user ID.
     */
    public ?string $user;

    /**
     * @param array{
     *   limit?: ?int,
     *   startingAfter?: ?string,
     *   endingBefore?: ?string,
     *   clientId?: ?string,
     *   email?: ?string,
     *   status?: ?value-of<GetApiV1UsersRequestStatus>,
     *   user?: ?string,
     * } $values
     */
    public function __construct(
        array $values = [],
    ) {
        $this->limit = $values['limit'] ?? null;
        $this->startingAfter = $values['startingAfter'] ?? null;
        $this->endingBefore = $values['endingBefore'] ?? null;
        $this->clientId = $values['clientId'] ?? null;
        $this->email = $values['email'] ?? null;
        $this->status = $values['status'] ?? null;
        $this->user = $values['user'] ?? null;
    }
}
